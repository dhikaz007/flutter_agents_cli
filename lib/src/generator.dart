...
class RuleGenerator {
  ...
  Future<GenerationReport> apply(
    ...
    if (!dryRun) {
      ...
      manifestStore.save(
        project,
        AgentsManifest(version: cliVersion, config: config.copy(), files: nextRecords),
      );
    }
    return report;
  }
  ...
  Future<Map<String, List<int>>> _desiredFiles(
    ...
    for (final key in config.profileKeys.toSet()) {
      ...
      final source = File(p.join(templates.path, 'profiles', sourceKind, '$name.md'));
      if (!source.existsSync()) continue;
      desired[ProfileRegistry.profilePath(kind, name)] = source.readAsBytesSync();
    }
  ...
  String _projectStack(StackConfig c) {
    ...
    final b = StringBuffer()
      ...
      ..writeln()
      ..writeln('<!-- GENERATED BY flutter-agents. Put manual conventions in docs/project/PROJECT-RULES.md. -->')
      ..writeln()
      ..writeln('## Project')
      ..writeln('- Mode: ${c.mode}')
      ..writeln()
    ...
    if (c.uiComponents.isEmpty) {
      b.writeln('- No explicit component map detected. Inspect existing shared/core widgets first.');
    } else {
      for (final entry in c.uiComponents.entries) {
        b.writeln('- ${entry.key}: ${entry.value == 'default' ? 'CLI default' : entry.value}');
      }
    }
    b
      ..writeln()
    ...
    b
      ...
      ..writeln('- Existing project: pubspec.yaml proves package availability; existing code proves active convention.')
      ..writeln('- New project: this stack declares intended choices; do not silently substitute packages.')
      ..writeln('- Manual project conventions belong in docs/project/PROJECT-RULES.md.');
    return b.toString();
  }

  void _ensureUserOwnedProjectRules(Directory project) {
    final file = File(p.join(project.path, 'docs', 'project', 'PROJECT-RULES.md'));
    if (file.existsSync()) return;
    file.parent.createSync(recursive: true);
    file.writeAsStringSync('''# Project Rules\n\n<!-- USER-OWNED. flutter-agents will not overwrite or uninstall this file. -->\n\nAdd only intentional project-specific conventions that cannot be inferred safely from code.\n\nExamples:\n- approved shared widget names\n- mutation/idempotency policy\n- project-specific Cubit lifecycle convention\n- API documentation precedence\n- design-system exceptions\n''');
  }
