...
class RuleGenerator {
  ...
  Future<GenerationReport> apply(
    ...
    if (!dryRun) {
      ...
      manifestStore.save(
        project,
        AgentsManifest(
            version: cliVersion, config: config.copy(), files: nextRecords),
      );
    }
    return report;
  }
  ...
  Future<Map<String, List<int>>> _desiredFiles(
    ...
    for (final key in config.profileKeys.toSet()) {
      ...
      final source =
          File(p.join(templates.path, 'profiles', sourceKind, '$name.md'));
  ...
  void _ensureUserOwnedProjectRules(Directory project) {
    final file =
        File(p.join(project.path, 'docs', 'project', 'PROJECT-RULES.md'));
