...
class PresetIO {
  ...
  Directory get userPresetsDir => Directory(p.join(_rules.root.path, 'presets'));
  ...
  List<String> userNames() {
    ...
    final out = userPresetsDir
        .listSync()
        .whereType<File>()
        .where((f) => <String>['.yaml', '.yml'].contains(p.extension(f.path).toLowerCase()))
  ...
  File userFile(String name) => File(p.join(userPresetsDir.path, '${_safe(name)}.yaml'));

  StackConfig? resolve(String nameOrPath) => resolveDocument(nameOrPath)?.config;
  ...
  PresetDocument? _readDocument(File file, {String? source}) {
    try {
      ...
      final ruleLayers = rawRules is Map ? rawRules.keys.map((e) => e.toString()).toSet() : <String>{};
      return PresetDocument(
  ...
  File saveUser(String name, StackConfig config, {String? description, bool overwrite = false}) {
  ...
  File saveUserDocument(String name, PresetDocument document, {bool overwrite = false}) {
    final file = userFile(name);
    if (file.existsSync() && !overwrite) {
      throw StateError('Preset already exists: ${p.basenameWithoutExtension(file.path)}');
    }
  ...
  void exportDocument(PresetDocument d, File file) {
    ...
    if (d.name != null && d.name!.trim().isNotEmpty) lines.add('name: ${_quote(d.name!.trim())}');
    ...
    if (d.fields.contains('uiComponents')) {
      ...
      if (d.config.uiComponents.isEmpty) {
        ...
        final entries = d.config.uiComponents.entries.toList()..sort((a, b) => a.key.compareTo(b.key));
